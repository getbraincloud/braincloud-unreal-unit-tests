...
void BrainCloudProduct::awardParentCurrency(const FString& currencyType, int32 amount, const FString& parentLevel, IServerCallback * callback)
{
    TSharedRef<FJsonObject> message = MakeShareable(new FJsonObject());

    message->SetStringField(OperationParam::ProductServiceVCId.getValue(), currencyType);
    message->SetNumberField(OperationParam::ProductServiceVCAmount.getValue(), amount);
    message->SetStringField(OperationParam::AuthenticationServiceAuthenticationLevelName.getValue(), parentLevel);

    ServerCall * sc = new ServerCall(ServiceName::Product, ServiceOperation::AwardParentVC, message, callback);
    _client->sendRequest(sc);
}

void BrainCloudProduct::consumeParentCurrency(const FString& currencyType, int32 amount, const FString& parentLevel, IServerCallback * callback)
{
    TSharedRef<FJsonObject> message = MakeShareable(new FJsonObject());
    ...
    ServerCall * sc = new ServerCall(ServiceName::Product, ServiceOperation::ConsumeParentVC, message, callback);
    _client->sendRequest(sc);
}

void BrainCloudProduct::getParentCurrency(const FString& currencyType, const FString& parentLevel, IServerCallback * callback)
{
    TSharedRef<FJsonObject> message = MakeShareable(new FJsonObject());
    ...
    ServerCall * sc = new ServerCall(ServiceName::Product, ServiceOperation::GetParentVC, message, callback);
    _client->sendRequest(sc);
}

void BrainCloudProduct::resetParentCurrency(const FString& parentLevel, IServerCallback * callback)
{
    TSharedRef<FJsonObject> message = MakeShareable(new FJsonObject());

    message->SetStringField(OperationParam::AuthenticationServiceAuthenticationLevelName.getValue(), parentLevel);

    ServerCall * sc = new ServerCall(ServiceName::Product, ServiceOperation::ResetParentVC, message, callback);
    _client->sendRequest(sc);
}