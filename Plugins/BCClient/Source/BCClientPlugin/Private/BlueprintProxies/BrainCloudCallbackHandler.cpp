...
void UBrainCloudCallbackHandler::RegisterCallbacks(UBrainCloudWrapper *brainCloudWrapper, bool fileCallbacks, bool rewardCallback,
                                                   bool eventCallback, bool globalErrorCallback, bool networkErrorCallback)
{
    ...
    if (fileCallbacks)
        bc->getBCClient()->registerFileUploadCallback(this);
    if (rewardCallback)
        bc->getBCClient()->registerRewardCallback(this);
    if (eventCallback)
        bc->getBCClient()->registerEventCallback(this);
    if (globalErrorCallback)
        bc->getBCClient()->registerGlobalErrorCallback(this);
